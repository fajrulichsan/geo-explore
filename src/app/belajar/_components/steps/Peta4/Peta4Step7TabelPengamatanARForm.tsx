...
const baris = [
  ...
  { key: "informasi_tambahan", label: "Informasi tambahan", type: "textarea" as const },
];
...
export default function Peta4Step7TabelPengamatanARForm({
  materi,
  peta,
  initialAnswers,
  geoGebraEntries,
  ...
  geoGebraEntries: Record<string, string>[];
  header: ReactNode;
}) {
  const initialEntries = (() => {
    const raw = initialAnswers.pengamatan_ar;
  ...
  const geoGebraEntry = geoGebraEntries.find((e) => e.model_diamati === draft.model_diamati);
  const geoGebraKeyMap: Record<string, string> = { informasi_tambahan: "catatan_lain" };
  function geoGebraValue(key: string) {
    if (!geoGebraEntry) return "";
    return geoGebraEntry[geoGebraKeyMap[key] ?? key] ?? "";
  }
    ...
    <form ref={formRef} action={submitStepAction} className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-4">
        ...
        <p className="m-0 text-[15px] leading-[1.6] text-[#374151] max-w-2xl">
          Catat informasi tambahan yang kamu peroleh melalui AR, lalu tambahkan ke daftar. Ulangi untuk
          setiap bangun ruang yang kamu amati. Pastikan data yang kamu catat lengkap dan jelas.
        </p>
      </div>
              ...
              {baris.map((b) => (
                <tr key={b.key} className="hover:bg-[#F9FAFB] transition-colors">
                  ...
                  <td className="px-5 py-3 border-l border-[#E5E7EB]">
                    {geoGebraValue(b.key) ? (
                      <div className="h-9 w-full bg-[#F3F4F6] rounded-md flex items-center px-3 text-xs font-medium text-[#374151]">
                        {geoGebraValue(b.key)}
                      </div>
                    ) : (
                      <div className="h-9 w-full bg-[#F3F4F6] rounded-md flex items-center px-3 text-xs italic text-[#9CA3AF]">
                        {draft.model_diamati ? "Belum ada data" : "Pilih bangun ruang terlebih dahulu..."}
                      </div>
                    )}
                  </td>
                  <td className="px-5 py-3 border-l border-[#E5E7EB]">
