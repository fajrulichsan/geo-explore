...
const fields = [
  {
    ...
    label: "Bagaimana kamu mengetahui bahwa hubungan yang kamu temukan sudah sesuai?",
    hint: "Jelaskan berdasarkan hasil verifikasi dan alasan matematis yang mendukung jawabanmu.",
  },
  {
...
export default function Materi5Peta8Step4CaraBerpikirku({ materi, peta, initialAnswers }: StepComponentProps) {
    ...
    <form action={submitStepAction} className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-[#F59E0B] text-white flex items-center justify-center font-bold text-sm flex-shrink-0">D</div>
          <h2 className="m-0 text-lg font-extrabold text-[#111827]">Refleksi Cara Berpikirku</h2>
        </div>
        ...
        {fields.map((f, i) => (
          ...
          >
            <div className="flex items-start gap-3">
              <span className="w-7 h-7 rounded-full bg-[#FDF3C7] text-[#92400E] flex items-center justify-center text-xs font-bold flex-shrink-0">
                {i + 1}
              </span>
              <label htmlFor={f.key} className="text-sm font-bold text-[#111827] leading-[1.6]">
                {f.label}
                {f.hint && <span className="block mt-1 font-semibold text-[#6B7280]">{f.hint}</span>}
              </label>
            </div>
            <textarea
              id={f.key}
