...
export default async function Materi3Peta1Step1TahukahKamu({ materi, peta, step = "1", editFoto }: StepComponentProps) {
  const gambarSiswa = await getPageImage("M3-P1-L1-1");
    ...
    <form action={submitStepAction} className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.1fr] gap-5 items-center">
          ...
          urutan="1"
          src={gambarSiswa}
          alt="Tiga siswa berdiskusi tentang luas permukaan dengan jaring-jaring kubus"
          editable={editFoto}
          natural
        ...
        <div className="bg-[#EFF4FF] border border-[#C7D7FE] rounded-2xl py-4 px-5 flex items-start gap-3">
          <div className="w-[30px] h-[30px] rounded-full bg-[#2563EB] text-white flex items-center justify-center font-bold text-base flex-shrink-0">
            ?
          </div>
          <p className="m-0 text-[15px] font-bold text-[#1D4ED8] leading-[1.5]">
            Menurutmu, bagaimana cara menentukan luas permukaan tanpa langsung menggunakan rumus?
          </p>
        </div>
      </div>
