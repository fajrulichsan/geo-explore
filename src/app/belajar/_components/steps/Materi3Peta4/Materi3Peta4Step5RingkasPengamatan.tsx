...
const lanjutkanChecklist = [
  "Rangkum semua hasil pengamatan.",
  "Lengkapi data penting yang kamu peroleh.",
  "Catat temuan sementara dari hasil eksplorasimu.",
  "Refleksikan proses eksplorasimu sebelum melanjutkan ke Tahap 4.",
];

const alurLanjutan = [
  { n: 3, label: "Catatan Hasil" },
  { n: 4, label: "Lengkapi Data" },
  { n: 5, label: "Temuan Sementara" },
  { n: 6, label: "Refleksi" },
];
...
const kolom = ["kubus", "balok", "prisma"] as const;
const kolomLabel: Record<(typeof kolom)[number], string> = { kubus: "Kubus", balok: "Balok", prisma: "Prisma" };

export default async function Materi3Peta4Step5RingkasPengamatan({
  materi,
  peta,
  initialAnswers,
}: StepComponentProps) {
          ...
          <table className="w-full border-collapse min-w-[640px]">
            <thead>
              <tr className="bg-[#F0FDF4]">
                ...
                {kolom.map((k) => (
                  <th key={k} className="text-left text-xs font-bold text-[#166534] p-4 border-b border-[#E5E7EB]">
                    {kolomLabel[k]}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {baris.map((b) => (
                <tr key={b.key} className="bg-white even:bg-[#F9FAFB]">
                  ...
                  {kolom.map((k) => (
                    <td key={k} className="p-4 border-b border-[#F3F4F6] align-top">
                      <input
                        type="text"
                        name={`answers.ringkas_${b.key}_${k}`}
                        defaultValue={getValue(`ringkas_${b.key}_${k}`)}
                        placeholder="..."
                        required
